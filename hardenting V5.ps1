...
function Initialize-Logging {
    ...
    if (-not (Test-Path $script:CsvPath)) {
        $csvHeader = "Timestamp_UTC,Hostname,OS_Version,Target_Version,Action,Result,ErrorMessage"
...
function Write-Log {
    ...
    $csvLine = "`"$timestamp`",`"$script:Hostname`",`"$script:OSVersion`",`"$script:TargetVersion`",`"$Action`",`"$Result`",`"$csvMessage`""
...
function Backup-Registry {
    ...
    try {
        $backupPath = "C:\SecurityHardening\RegistryBackup_$(Get-Date -Format 'yyyyMMdd_HHmmss').reg"
...
function Set-TLSHardening {
    ...
    foreach ($setting in $tlsSettings) {
        try {
            ...
            Write-Log -Action $setting.Description -Result "Success" -Message "Path: $($setting.Path), Value: $($setting.Value)"
        }
        catch {
